...
for i=1:ncell
    si{i}=zeros(size(Cell.Edges{i}*4*3,1),1);
    sj{i}=si{i};
    sv{i}=si{i};
    sk{i}=0;
end
...
parfor i=1:ncell
    ...
    for e=1:size(Edges,1)
        if Edges(e,1)<=Y.n, Y1=Y.DataRow(Edges(e,1),:); else,  Y1=CellSurfsCenters.DataRow(Edges(e,1)-Y.n,:); end 
        if Edges(e,2)<=Y.n, Y2=Y.DataRow(Edges(e,2),:); else,  Y2=CellSurfsCenters.DataRow(Edges(e,2)-Y.n,:); end 
        if Edges(e,3)<=Y.n, Y3=Y.DataRow(Edges(e,3),:); else,  Y3=CellSurfsCenters.DataRow(Edges(e,3)-Y.n,:); end 
        if Edges(e,4)<=Y.n, Y4=Y.DataRow(Edges(e,4),:); else,  Y4=CellSurfsCenters.DataRow(Edges(e,4)-Y.n,:);end 
...
%         Ke=fact2*dnndy*(dnndy')+  fact0*Knn;
        ...
        % AssembleK
%         if Set.Sparse
            [si{i},sj{i},sv{i},sk{i}]= AssembleKTriangleSAreaSparse(Ke,Edges(e,:),si{i},sj{i},sv{i},sk{i});
%         else
