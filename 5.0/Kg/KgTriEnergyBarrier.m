...
%        fact2=fact*-((lambdaB*Set.Beta)/Cell.SAreaTri0{i}(t));
%         nY=Tris(t,:);
%         Y1=Y(nY(1),:);
%         Y2=Y(nY(2),:);
...
%         if Set.Sparse
%             K=K+sparse((ge)*(ge')*fact2);
%         else
%             K=K+(ge)*(ge')*fact2 ;  %-(gee)*(gee')*(fact);
%         end
%         EnergyB=EnergyB+ exp(lambdaB*(1-Set.Beta*Cell.SAreaTri{i}(t)/Cell.SAreaTri0{i}(t)));
%     end 
...
%         Cell.SAreaTri0{i}(t)=1e-4;
       ...
       fact2=fact*-((lambdaB*Set.Beta)/Cell.SAreaTri0{i}(t));
        ...
        g=AssemblegTriangleSArea(g,gs*fact,nY);
        EnergyB=EnergyB+ exp(lambdaB*(1-Set.Beta*Cell.SAreaTri{i}(t)/Cell.SAreaTri0{i}(t)));
    end 
end
