function [Cell,Y,Yn,SCn,X,Dofs,Set,Energy,DidNotConverge]=SolveRemodelingStep(Cell,T,Y0, Y, X, Dofs, Set, Yn, Cn, SCn, CellInput)
...
while 1
    ...
    [g,~,Cell, Y, Energy, Set, gr, dyr, dy] = newtonRaphson(Set, Cell,Cn, SCn, K, g, Dofs,T, X, Y, Y0, Yn, CellInput, -1, -1, 1);
    ...
    elseif gr>Set.tol || dyr>Set.tol || any(isnan(g(Dofs.FreeDofs))) || any(isnan(dy(Dofs.FreeDofs))) || Set.nu/Set.nu0 ~=1
        ...
        error('Local Problem did not converge after %i iterations.\n',Set.iter);
