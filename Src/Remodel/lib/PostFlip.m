...
if CheckTris(Geo) %%&& ~CheckConvexity(Tnew,Geo_backup)
    ...
    if DidNotConverge
        ...
        Geo_0 = Geo_0_backup;
        Dofs = Dofs_backup;
        Geo.log = strcat(Geo.log, sprintf('=>> %s-Flip rejected: did not converge\n', flipName));
        return
    end
    ...
    Dofs = Dofs_backup;
    Geo_0 = Geo_0_backup;
    Geo.log = strcat(Geo.log, sprintf('=>> %s-Flip rejected: is not compatible\n', flipName));
    return
end
end
