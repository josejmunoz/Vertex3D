...
%                 if ~gettingCloser
%                     Geo.Cells(idCell).Y(ismember(sort(Geo.Cells(idCell).T, 2), tetToCheck', 'rows'), :) = newPoint + (avgPoint *  (distanceToRefPointByCell_min / pdist2(newPoint, refPoint_further))^(1));
%                 end
%             end
%         end
%     end
